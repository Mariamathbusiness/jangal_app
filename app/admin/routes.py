...
# --- GESTION DES ÉCOLES (CRÉATION & MODIFICATION) ---
...
def manage_schools():
    ...
    # 1. Mode Édition : on pré-remplit le formulaire
    edit_id = request.args.get('edit', type=int)
    if edit_id and not form.is_submitted():
        query = "SELECT * FROM schools WHERE id = ?"
        cursor, conn = execute_query(query, (edit_id,))
        ...
        if school:
            ...
            form.director_name.data = school.get('director_name', '')
            form.address.data = school.get('address', '')
            form.phone.data = school.get('phone', '')
            form.email.data = school.get('email', '')
            form.start_date.data = school.get('start_date')
            form.end_date.data = school.get('end_date')
            form.status.data = school.get('status', 'active')
    ...
    if form.validate_on_submit():
        ...
        if school_id:
            # --- MODIFICATION ---
            query = """UPDATE schools SET name = ?, director_name = ?, address = ?, 
                       phone = ?, email = ?, start_date = ?, end_date = ?, status = ?
                       WHERE id = ?"""
            params = (form.name.data, form.director_name.data, form.address.data, 
                      form.phone.data, form.email.data, form.start_date.data, 
                      form.end_date.data, form.status.data, school_id)
            ...
            query = """INSERT INTO schools (uuid, name, director_name, address, phone, email, start_date, end_date, status)
                       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)"""
            params = (str(uuid.uuid4()), form.name.data, form.director_name.data, form.address.data, 
                      form.phone.data, form.email.data, form.start_date.data, form.end_date.data, form.status.data)
    ...
    # 3. Récupération de la liste des écoles pour l'affichage
    query = "SELECT id, name, director_name, start_date, end_date, status FROM schools ORDER BY id ASC"
...
# --- SUPPRESSION D'UNE ÉCOLE (SÉCURISÉE) ---
@admin_bp.route('/super_admin/schools/delete/<int:school_id>')
@login_required
@super_admin_required
def delete_school(school_id):
    # Empêcher la suppression de l'école principale (ID 1) par sécurité
    if school_id == 1:
        flash("⚠️ L'établissement principal (ID 1) ne peut pas être supprimé.", "danger")
    else:
        # Vérifier si l'école a des utilisateurs avant de supprimer (Sécurité Multi-tenancy)
        query_check = "SELECT COUNT(id) as count FROM users WHERE school_id = ?"
        cursor, conn = execute_query(query_check, (school_id,))
        if cursor.fetchone()['count'] > 0:
            flash("⚠️ Impossible de supprimer : cet établissement contient des utilisateurs. Veuillez le passer en statut 'Suspendu' ou 'Expiré' plutôt.", "danger")
            conn.close()
            return redirect(url_for('admin.manage_schools'))
        
        query_delete = "DELETE FROM schools WHERE id = ?"
        cursor, conn = execute_query(query_delete, (school_id,))
        conn.commit()
        conn.close()
